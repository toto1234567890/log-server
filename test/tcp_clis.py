...
def client_worker(client_id):
    """Worker function for each client process"""
    ...
    try:
        with socket.create_connection(("127.0.0.1", 9020)) as sock:
            ...
            while True:
                serialized_message = send_capnp_message(client_id)
                logSock.send_data(serialized_message)
                message_count += 1
                print(f"Client {client_id}: Sent message #{message_count}")
                sleep(0.01)
...
if __name__ == "__main__":
    ...
    for i in range(NUM_CLIENTS):
        process = multiprocessing.Process(target=client_worker, args=(i,))
        processes.append(process)
        process.start()
        print(f"Started client process {i}")
        sleep(0.1)  # Small delay to stagger connections
    
    print("All client processes started. Press Ctrl+C to stop.")
